# Document 08: Master Dashboard (National Overview)
...
Build the comprehensive Master Dashboard for Ministry leadership and policy makers. This is the **most complete dashboard** showing ALL metrics across the entire Madares Score system - schools, evaluations, compliance, appeals, and trends.

**Component:** `src/pages/Master/Dashboard.jsx`

---

## UI Layout with Complete KPIs

```
┌──────────────────────────────────────────────────────────────┐
│ SYSTEM-WIDE KPI CARDS (Top Row - 6 cards)                    │
│ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌───┐│
│ │ Total  │ │Active  │ │  Avg   │ │Schools │ │ Active │ │SLA││
│ │Schools │ │Evals   │ │ Score  │ │ Above  │ │Appeals │ │⚠️ ││
│ │ 1,234  │ │  156   │ │ 82.5%  │ │  A/A+  │ │   24   │ │5% ││
│ │        │ │        │ │  (B)   │ │  287   │ │        │ │   ││
│ └────────┘ └────────┘ └────────┘ └────────┘ └────────┘ └───┘│
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ COMPLIANCE KPI CARDS (Second Row - 5 cards)                  │
│ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐     │
│ │Complnt │ │  Not   │ │License │ │ Safety │ │Finncl  │     │
│ │ Rate   │ │Complnt │ │ Exp    │ │ Viol.  │ │ Issues │     │
│ │ 94.2%  │ │   72   │ │   15   │ │   28   │ │   8    │     │
│ └────────┘ └────────┘ └────────┘ └────────┘ └────────┘     │
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ EXCELLENCE & SATISFACTION METRICS (Third Row - 6 cards)      │
│ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌───┐│
│ │  Avg   │ │Teacher │ │Student │ │  Avg   │ │Parent  │ │Bul││
│ │Excllnce│ │ Qual.  │ │-Tchr   │ │Satisf. │ │ Satisf.│ │Idx││
│ │ 81.3%  │ │ 87.5%  │ │  15:1  │ │ 84.7%  │ │ 86.2%  │ │95%││
│ └────────┘ └────────┘ └────────┘ └────────┘ └────────┘ └───┘│
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ MAP & CHARTS (Main Content Area)                             │
│ ┌────────────────────┐ ┌────────────────────┐               │
│ │ Geographic Heat Map│ │ Grade Distribution │               │
│ │ (By Region)        │ │ (Bar Chart)        │               │
│ └────────────────────┘ └────────────────────┘               │
│ ┌────────────────────┐ ┌────────────────────┐               │
│ │ Trends Over Time   │ │ Top/Bottom Schools │               │
│ │ (Line Chart)       │ │ (Table)            │               │
│ └────────────────────┘ └────────────────────┘               │
└──────────────────────────────────────────────────────────────┘
...
│ Region | Schools | Avg Score | A+/A | B+/B | C+/C | D+/D/F │
└──────────────────────────────────────────────────────────────┘
```
...
import { mockSchools, mockEvaluations, mockIndicators, getGradeBand } from '../../data/mockData';
...
### System-Wide KPIs (Row 1)
1. **Total Schools** - All schools in system
2. **Active Evaluations** - Currently in progress/review
3. **Avg National Score** - Weighted average with grade
4. **Top Performers** - Schools with A+/A grades
5. **Active Appeals** - Under review
6. **SLA Compliance** - Percentage meeting deadlines

### Compliance KPIs (Row 2)
7. **Compliance Rate** - % of compliant schools
8. **Non-Compliant** - Schools failing compliance
9. **License Expiring** - Within 3 months
10. **Safety Violations** - Unresolved violations
11. **Financial Issues** - Compliance problems

### Excellence & Satisfaction KPIs (Row 3)
12. **Avg Excellence** - Institutional excellence score
13. **Teacher Quality** - % qualified teachers
14. **Student-Teacher Ratio** - Average ratio
15. **Avg Satisfaction** - Overall satisfaction
16. **Parent Satisfaction** - Parent survey results
17. **Safety Index** - Bullying/incident rate

---

## Testing Checklist

- [ ] All 17 KPI cards display correctly
- [ ] Calculations accurate across all metrics
- [ ] Trend indicators show correct direction
- [ ] Grade distribution chart accurate
- [ ] Performance insights show top/bottom schools
- [ ] Regional breakdown table calculates correctly
- [ ] Region averages accurate
- [ ] Grade counts per region correct
- [ ] Quick action buttons navigate correctly
- [ ] All color coding appropriate (green/yellow/red)
...
2. ✅ Verify regional aggregations
3. ✅ Update INDEX.md with new documents
4. ✅ Update existing documents with KPIs
